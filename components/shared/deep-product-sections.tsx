...
import { CheckCircle2, BarChart3, TrendingUp, Zap, Globe, Shield, Activity, Users, MonitorPlay } from "lucide-react";

export function DeepProductSections() {
  return (
    <div className="bg-[#0B0B0C]">
      
      {/* 6. CORE PLATFORM SECTION (VERY IMPORTANT) */}
      <section className="py-40 relative overflow-hidden bg-[#0B0B0C]">
        {/* Background Grid */}
        ...
        <div className="max-w-[1200px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-24 items-center">
          ...
          >
            <div className="inline-flex items-center gap-3 text-[#F5C044] text-[12px] font-bold tracking-[0.3em] uppercase mb-10">
              <div className="w-1.5 h-1.5 rounded-full bg-[#F5C044]" />
              Core Platform
            </div>
            <h2 className="text-[42px] md:text-[56px] font-bold text-white tracking-tight leading-[1.05] mb-8">
              The engine for <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-[#F5C044]">enterprise operations</span>
            </h2>
            <p className="text-[19px] text-[#A1A1AA] leading-relaxed mb-10 font-light">
              OYEN GRID isn't just a tool; it's the operational substrate for your most critical programmes. Built to handle complexity without sacrificing speed.
            </p>
            <div className="space-y-6">
              {[
                ...
                <div key={b} className="flex items-center gap-4 group">
                  <div className="w-6 h-6 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-[#F5C044]/10 group-hover:border-[#F5C044]/30 transition-all">
                    <CheckCircle2 className="w-3.5 h-3.5 text-[#F5C044]" />
                  </div>
                  <span className="text-[#D4D4D8] text-[16px] group-hover:text-white transition-colors">{b}</span>
                </div>
              ))}
            </div>
          </motion.div>

          {/* Right: COMPLEX DASHBOARD SCREENSHOT */}
          <motion.div
          ...
          >
            <div className="absolute -inset-10 bg-blue-500/10 blur-[100px] rounded-full opacity-50" />
            <div className="relative aspect-[16/10] bg-[#0D0D10] border border-white/10 rounded-[32px] shadow-[0_60px_120px_rgba(0,0,0,0.8)] overflow-hidden ring-1 ring-white/10 group">
              <Image 
                src="/images/hero-dashboard.png" 
                alt="Complex Platform UI" 
                fill
                className="object-cover opacity-90 group-hover:scale-105 transition-transform duration-1000"
              />
              {/* Complex Overlay details */}
              <div className="absolute top-8 right-8 p-4 rounded-2xl bg-[#0D0D10]/80 border border-white/10 backdrop-blur-xl shadow-2xl flex items-center gap-4">
                 <div className="w-10 h-10 rounded-full border-2 border-emerald-500/30 flex items-center justify-center text-[10px] font-bold text-emerald-400">92%</div>
                 <div>
                   <p className="text-white font-bold text-xs">Uptime</p>
                   <p className="text-[#52525B] text-[10px]">Real-time Status</p>
                 </div>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-6">
              {[
                ...
                <div key={stat.label} className="p-8 rounded-[28px] bg-white/[0.03] border border-white/10 hover:bg-white/[0.05] transition-all group">
                  <p className="text-[#71717A] text-xs font-bold uppercase tracking-widest mb-4 group-hover:text-[#F5C044] transition-colors">{stat.label}</p>
            ...
            <div className="relative p-10 bg-[#0D0D10] border border-white/10 rounded-[32px] shadow-[0_60px_100px_rgba(0,0,0,0.8)] overflow-hidden ring-1 ring-white/10 group">
              <div className="flex items-center justify-between mb-12">
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10">
                  <Activity className="w-4 h-4 text-[#F5C044] animate-pulse" />
                  <span className="text-[#F5C044] text-[10px] font-bold uppercase tracking-widest">Live Now</span>
                </div>
              </div>
                  ...
                  <div key={i} className="flex-1 flex flex-col items-center gap-3 group/bar">
                    <motion.div 
                      ...
                      className={`w-full rounded-t-xl transition-all duration-500 relative ${
                        i === 7 ? "bg-[#F5C044] shadow-[0_0_30px_rgba(245,192,68,0.4)]" : "bg-white/10 group-hover/bar:bg-white/20"
                      }`}
                    >
      ...
      {/* 8. FEATURE GRID (EVERYTHING YOU NEED TO SCALE) */}
      <section className="py-40 relative">
        <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_50%_50%,rgba(245,192,68,0.03)_0%,transparent_70%)]" />
        
        <div className="max-w-[1200px] mx-auto px-6 relative z-10">
          <div className="text-center mb-28">
            <h2 className="text-[36px] md:text-[52px] font-bold text-white tracking-tight">
              Everything you need <br />
              <span className="text-[#F5C044]">to operate at scale</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <motion.div 
                ...
                className="p-10 rounded-[32px] bg-white/[0.02] border border-white/10 hover:border-[#F5C044]/30 hover:bg-white/[0.04] transition-all group shadow-xl"
              >
                <div className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center mb-8 group-hover:scale-110 group-hover:bg-[#F5C044]/10 transition-all duration-500 border border-white/5">
