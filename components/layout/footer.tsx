    ...
    <footer className="relative text-white mt-12">
      ...
      <div className="w-full overflow-hidden leading-[0] bg-transparent">
        <svg viewBox="0 0 1440 60" preserveAspectRatio="none" className="relative block w-full h-[60px]">
          ...
          <path d="M0,60 L0,10 C400,10 700,50 1000,50 C1200,50 1350,20 1440,20 L1440,60 Z" fill="#08111F" />
          <path d="M0,10 C400,10 700,50 1000,50 C1200,50 1350,20 1440,20" fill="none" stroke="url(#footer-curve-grad)" strokeWidth="1.5" />
        </svg>
      </div>

      {/* Main Footer Body */}
      <div className="bg-[#08111F] pt-8 pb-10">
                  ...
                  <ul className="flex flex-col gap-2">
                    {col.links.map((link) => (
                      <li key={link.label}>
                        <Link
                          href={link.href}
                          className="text-[14px] text-[#98A2B3] hover:text-[#C99A20] transition-colors duration-250 font-normal leading-tight cursor-pointer"
                        >
                          {link.label}
                        </Link>
                      </li>
            ...
            <div className="flex flex-col items-start gap-2.5">
              ...
              <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-2.5 w-full mt-1">
                <input
                  ...
                  className="h-[42px] px-4 rounded-xl bg-white/[0.03] border border-white/10 text-white placeholder-white/30 text-[13px] focus:outline-none focus:border-[#C99A20]/40 transition-colors w-full sm:max-w-[240px]"
                />
                <button
                  type="submit"
                  className="h-[42px] px-5 rounded-xl bg-[#FFFFFF] text-[#08111F] font-bold text-[12px] uppercase tracking-wider transition-all duration-300 hover:bg-[#F4F1EA] hover:shadow-[0_4px_16px_rgba(255,255,255,0.1)] hover:scale-[1.02] cursor-pointer"
                >
                  Subscribe
                </button>
              </form>
          ...
          <div className="pt-6 border-t border-white/[0.04] flex items-center justify-start gap-3">
            {socialLinks.map((s) => (
              <Link
                ...
                href={s.href}
                aria-label={s.label}
                className="w-8.5 h-8.5 rounded-lg bg-white/[0.02] border border-white/5 flex items-center justify-center text-[#98A2B3] hover:text-[#C99A20] hover:border-[#C99A20]/30 transition-all duration-300 cursor-pointer"
              >
                {s.icon}
              </Link>
            ))}
            ...
            <div className="flex items-center gap-6">
              {["Privacy Policy", "Terms of Service"].map((item) => (
                <Link
                  key={item}
                  href="#"
                  className="text-[12px] text-[#98A2B3] hover:text-[#C99A20] transition-colors duration-250 font-light cursor-pointer"
                >
                  {item}
                </Link>
              ))}
