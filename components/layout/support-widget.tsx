  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] font-sans antialiased text-white selection:bg-[#FFC72C]/30">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-4 w-[310px] h-[460px] flex flex-col bg-[#0B0B0B] border border-[#222222] rounded-2xl overflow-hidden shadow-[0_24px_60px_rgba(0,0,0,0.85)]"
          >
            {/* Header */}
            <div className="px-3.5 py-2 bg-[#111111] border-b border-[#222222] flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-0.5">
                {messages.length > 1 && (
                  <button
                    onClick={resetChat}
                    className="p-1 rounded-lg hover:bg-[#222222] text-zinc-500 hover:text-zinc-300 transition-colors"
                    title="Start over"
                  >
            ...
            {/* Chat Feed */}
            <div className="flex-1 overflow-y-auto p-3 space-y-3.5 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent">
              {messages.map((msg) => (
                <div
                  key={msg.id}
                ...
                >
                  {/* Bubble */}
                  <div
                    className={`max-w-[88%] rounded-2xl px-3 py-2 text-[11px] leading-relaxed shadow whitespace-pre-line ${
                      msg.sender === "user"
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2 max-w-[92%]">
                      {msg.options.map((opt) => (
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded-lg border border-[#2a2a2a] bg-[#161616] hover:bg-[#FFC72C] hover:text-black hover:border-[#FFC72C] text-zinc-300 text-[9px] font-bold tracking-wide uppercase transition-all duration-200 shadow active:scale-95"
                        >
                          {opt}
                        </button>
                      ))}
              ...
              {messages.length === 1 && (
                <div className="flex flex-col gap-1.5 pt-1.5 max-w-[88%]">
                  {MAIN_MENU_ACTIONS.map((action) => (
                    <button
                      key={action}
                      onClick={() => handleQuickAction(action)}
                      className="w-full text-left px-3 py-2 rounded-lg border border-[#222222] bg-[#141414] hover:border-[#FFC72C]/50 hover:bg-[#FFC72C]/5 text-zinc-200 hover:text-white text-[10px] font-semibold transition-all duration-200 flex items-center justify-between group"
                    >
                      <span>{action}</span>
                      <ChevronRight className="w-3 h-3 text-[#FFC72C] opacity-0 group-hover:opacity-100 transition-opacity" />
                    </button>
                  ))}
                </div>
              )}
            ...
            {/* Message Input */}
            <div className="p-2 bg-[#111111] border-t border-[#1a1a1a] shrink-0">
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  handleSend();
                }}
                className="relative flex items-center bg-[#0B0B0B] border border-[#222222] focus-within:border-[#FFC72C]/40 rounded-2xl px-2.5 py-1.5 transition-all"
              >
                <input
                  type="text"
                  value={inputValue}
                  ...
                  className="flex-1 bg-transparent text-[11px] text-white placeholder-zinc-600 focus:outline-none disabled:cursor-not-allowed pr-8"
                />
                <button
                  type="submit"
                  disabled={!isQuestionMode || !inputValue.trim()}
                  className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-xl bg-[#1e1e1e] text-zinc-500 enabled:hover:bg-[#FFC72C] enabled:hover:text-black disabled:opacity-25 transition-all duration-150 active:scale-90"
            ...
            {/* Footer */}
            <div className="bg-[#0B0B0B] py-1.5 border-t border-[#1a1a1a] text-center select-none shrink-0 flex flex-col gap-0.5">
              <span className="text-[8px] font-semibold tracking-wider text-zinc-600 uppercase">
                Need help? Contact our team.
              </span>
              <span className="text-[7.5px] text-zinc-700">
      ...
      <button
        ...
        className="w-12 h-12 rounded-full bg-[#0B0B0B] hover:bg-[#141414] border border-[#272727] hover:border-[#FFC72C]/40 text-[#FFC72C] flex items-center justify-center relative shadow-2xl transition-all duration-200 group active:scale-95"
      ...
      >
        ...
        {/* Online dot */}
        <span className="absolute bottom-0.5 right-0.5 w-2.5 h-2.5 bg-emerald-500 border-2 border-[#0B0B0B] rounded-full" />
      </button>
    </div>
  );
}
