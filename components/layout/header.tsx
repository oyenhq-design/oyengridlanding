...
export function Header() {
  ...
  return (
    <header className={cn(
      ...
      scrolled 
        ? "bg-[rgba(4,6,12,0.72)] backdrop-blur-[18px] h-[68px] border-b border-white/5 shadow-2xl" 
      ...
      <div className="max-w-[1240px] mx-auto h-full flex items-center justify-between px-6 relative z-10">
        ...
        <div className="flex items-center gap-10">
          <Link href="/" className="flex items-center gap-3">
            ...
            <span className="font-bold text-white text-[16px] tracking-tight">OYEN GRID</span>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navItems.map((item) => (
              <Link 
                key={item.label} 
                href="#" 
                className="text-[13px] font-medium text-white/90 hover:text-brand-gold transition-all flex items-center gap-1.5 group tracking-tight"
              >
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-5 pr-6 border-r border-white/5">
             ...
             >
              ...
              <Link href="#" className="text-[13px] font-medium text-white/90 flex items-center gap-1.5 hover:text-brand-gold group">
                Meet <ChevronDown className="w-3 h-3 text-white/30 group-hover:text-brand-gold" />
              </Link>
              <Link href="#" className="text-[13px] font-medium text-white/70 hover:text-white transition-colors">Sign In</Link>
              <Link href="#" className="text-[13px] font-medium text-white/70 hover:text-white transition-colors">Support</Link>
          </div>
          
          <div className="flex items-center gap-4">
            <button className="h-[36px] px-5 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-[12px] font-bold transition-all hover:bg-white/[0.08] hover:border-brand-gold/30 hover:text-brand-gold">
              Contact Sales
            </button>
            <button className="h-[36px] px-5 rounded-lg bg-brand-gold text-brand-dark text-[12px] font-black transition-all hover:shadow-[0_0_20px_rgba(245,185,66,0.4)] hover:scale-[1.02]">
              Sign Up Free
            </button>
...
export function AnnouncementBar() {
  ...
  return (
    <div className="pt-[80px] px-4 md:px-10 flex justify-center relative z-[60]">
      <motion.div 
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        className="h-[44px] bg-[#0E1424] border border-white/[0.06] rounded-[14px] px-5 flex items-center justify-between w-full max-w-[1240px] shadow-2xl"
      >
